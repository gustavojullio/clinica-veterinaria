...
<body>
	<c:choose>
		<c:when test="${autenticado}">
			...
			<h2>Informações do Veterinário</h2>
			<br>
			<h3>
				<b>${veterinario.getNome()}</b>
			</h3>
			<p>
				<b>Idade: </b>${veterinario.getIdade()}</p>
			<p>
				<b>Telefone: </b>${veterinario.getTelefone()}</p>
			<p>
				<b>Usuário: </b>${veterinario.getUsuario()}</p>
			<hr>
			<br>
			<a href="/ClinicaVeterinaria/VeterinariosController?opcao=listVet"><button
					type="button" class="btn btn-danger">Voltar</button></a>
			<a
				href="/ClinicaVeterinaria/AnimaisController?opcao=filtro&id=${veterinario.id}"><button
					type="button" class="btn btn-primary">Listar Animais
					Atendidos</button></a>
		</c:when>
		<c:otherwise>
			<header>
